...
def create_Car():
    ...
    if not request.json:
        abort(400)
    #If the reservation value is anything other than 1 or 0, alert the user
    if not (request.json['availability'] == 0 or request.json['availability'] == 1):
        return "The availability should be either 1 or 0", 400
    #if the request is made without an id, alert the user
    if 'vehicule_id' not in request.json:
        abort(400, "Missing 'vehicule_id' in the JSON data.")   
    #condition: as long as the int place holders receive int values, everything is fine 
    condition = isinstance(request.json['vehicule_id'],int ) and isinstance(request.json['seats'],int ) and isinstance(request.json['model_year'],int ) and  isinstance(request.json['price'],int ) and isinstance(request.json['availability'],int )
    if  condition:
        ...
        return "Make sure that the values entered into the following categories are integers: vehicule_id, seats, make_year, availability, price", 400
